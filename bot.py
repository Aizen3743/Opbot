...
def get_user_balance_text(user_id: int) -> str:
    conn = db()
    rows = conn.execute(
        """
        SELECT currency, network, SUM(amount) AS total
        FROM proofs
    ...
    for row in rows:
        ...
        grouped[currency].append((network, total))
    ...
    for currency, networks in grouped.items():
        lines.append(f"{currency} :-")

        for network, amount in networks:
            lines.append(f"{network} : {decimal_text(amount)} {currency}")
...
def verify_timer_text(currency: str, network: str, remaining: int) -> str:
    return (
        ...
        f"Transaction must be newer than 5 minutes.\n"
...
def validate_tx_age(timestamp_seconds: Optional[int]) -> Tuple[bool, str]:
    if not timestamp_seconds:
        return False, "Could not read transaction time. Try again after confirmation."

    now = int(time.time())
    age = now - int(timestamp_seconds)

    if age < 0:
        return False, "Transaction timestamp is invalid."

    if age > MAX_TX_AGE_SECONDS:
        return False, "Transaction is older than 5 minutes. Verification denied."

    return True, "Transaction time accepted."
...
def verify_tron_token_from_hash(
    ...
    timestamp_ms = data.get("timestamp") or data.get("block_timestamp")
    timestamp_seconds = None

    if timestamp_ms:
        timestamp_seconds = int(timestamp_ms) // 1000

    age_ok, age_reason = validate_tx_age(timestamp_seconds)

    if not age_ok:
        return False, age_reason, None
...
def bsc_rpc_call(method: str, params: list) -> dict:
    payload = {
        "jsonrpc": "2.0",
        "id": 1,
        "method": method,
...
def get_bsc_block_timestamp(block_number_hex: str) -> Optional[int]:
    try:
        data = bsc_rpc_call("eth_getBlockByNumber", [block_number_hex, False])
...
def verify_bsc_token_from_hash(
    ...
    try:
        data = bsc_rpc_call("eth_getTransactionReceipt", [tx_hash])
    except Exception as e:
    ...
    timestamp_seconds = get_bsc_block_timestamp(block_number)
...
def verify_erc20_from_hash(
    ...
    for t in result:
        ...
        timestamp_seconds = int(t.get("timeStamp") or "0")
        age_ok, age_reason = validate_tx_age(timestamp_seconds)
...
def verify_btc_from_hash(
    ...
    age_ok, age_reason = validate_tx_age(block_time)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    balance_text = get_user_balance_text(user.id)
...
async def menu_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "menu:home":
        balance_text = get_user_balance_text(q.from_user.id)
    ...
    if action == "deposit":
        ...
        await q.edit_message_text(
            ...
            f"{balance_text}\n\n"
            f"Please contact owner: {OWNER_USERNAME}",
...
async def proof_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    balance_text = get_user_balance_text(user_id)

    await verifying_message.edit_text(
        f"Verification done.\n\n"
        f"Coin: {currency}\n"
        f"Network: {network}\n"
        f"Amount added: {decimal_text(detected_amount)} {currency}\n\n"
        f"{balance_text}"
    )
...
def fallback_get_new_payments_from_old_wallet_notifier() -> list:
    ...
    for tx in wallet_notifier.fetch_btc(btc_addr):
        ...
        key = wallet_notifier._safe_key("BITCOIN", "BITCOIN", txid, str(amount_btc))

        if wallet_notifier.seen(key):
            continue

        wallet_notifier.mark_seen(key, txid, "BITCOIN", "BITCOIN", str(amount_btc), tx)
