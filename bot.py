...
# Etherscan V2 uses ONE API key for Ethereum, BSC, and supported chains.
# You can set only ETHERSCAN_API_KEY.
# BSCSCAN_API_KEY is kept so old VPS env setup will not break.
...
async def menu_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "deposit":
        ...
        await q.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("⬅️ Back", callback_data="menu:withdraw")]]
            ),
        )
