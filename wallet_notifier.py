...
def notify(bot: Bot, text: str) -> None:
    if BOT_TOKEN and OWNER_ID:
        bot.send_message(chat_id=OWNER_ID, text=text)


def loop() -> None:
    if not BOT_TOKEN or not OWNER_ID:
        raise RuntimeError("Set BOT_TOKEN and OWNER_ID")

    bot = Bot(BOT_TOKEN)
    init_tables()

    while True:
        # TRON example for USDT TRC20
        ...
        for tx in fetch_tron_trc20(tron_addr):
            ...
            notify(bot, f"🔔 New incoming payment detected\n{symbol} on TRON\nAmount: {amount}\nTX: {tx_hash}")

        # BTC simple incoming tx detector
